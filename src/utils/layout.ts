...
export interface BarLayout {
  /** Высота полосы в пикселях (фиксированная ~1/3 rowHeight). */
  barHeight: number;
  /** Вертикальный отступ от верхнего края строки. */
  yOffset: number;
  /** Фактическая высота строки (может быть больше rowHeight при пересечениях). */
...
function overlaps(a: GanttBar, b: GanttBar): boolean {
  return (
    a.startX < b.startX + b.width - EPS && a.startX + a.width > b.startX + EPS
  );
}
 ...
 * 3. Для каждой компоненты жадно распределяем полосы по «подстрокам» (слотам).
 * 4. Высота каждой полосы фиксирована = rowHeight / 3.
 * 5. Если слотов > 3, увеличиваем actualRowHeight, чтобы все влезли.
 * 6. Самая длинная полоса размещается в центральном слоте.
 */
export function computeLayout(
  ...
  for (const [, rowBars] of byRow) {
    ...
    for (const startBar of rowBars) {
      ...
      // Фиксированная высота полоски — примерно 1/3 от базовой высоты строки.
      const barHeight = Math.round(rowHeight / 3);
      const gap = 4; // зазор между полосками

      // Одиночная полоса без наложений — центрируем в строке.
      if (component.length === 1 && adj.get(component[0].id)!.length === 0) {
        result.set(component[0].id, {
          barHeight,
          yOffset: (rowHeight - barHeight) / 2,
      ...
      // сортируем по startX и присваиваем каждой полосе первый свободный слот.
      ...
      const numSlots = slotEndX.length;
      // Если слотов <= 3, используем базовую высоту строки.
      // Если больше — расширяем строку.
      const actualRowHeight = Math.max(
        rowHeight,
        numSlots * (barHeight + gap) + gap,
      );

      // Находим самую длинную полоску — она пойдёт в центральный слот.
      const longest = component.reduce((max, bar) =>
        bar.width > max.width ? bar : max,
      );
      const longestSlot = subRows.get(longest.id)!;
      const centerSlot = Math.floor((numSlots - 1) / 2);

      // Если самая длинная полоска не в центре — меняем слоты местами.
      if (longestSlot !== centerSlot) {
        for (const [barId, slot] of subRows) {
          if (slot === longestSlot) {
            subRows.set(barId, centerSlot);
          } else if (slot === centerSlot) {
            subRows.set(barId, longestSlot);
          }
        }
      }

      // Вычисляем yOffset для каждой полоски.
      for (const bar of component) {
        const slot = subRows.get(bar.id) ?? 0;
        const yOffset = gap + slot * (barHeight + gap);
        result.set(bar.id, {
          barHeight,
          yOffset,
          actualRowHeight,
